...
#let generateAbbrFromYaml(contents) = {
  ...
  if (contents != none) {
    for (word, definition) in contents {
      let w = [#figure(kind: "word_definition", supplement: "Definizione")[*#upper(word.at(0))#word.slice(1) :*]#label(
          word + "abbr",
        )]
          ...
          while i <= counter(word + "abbr").get().at(0) {
            ...
            if (pages.contains(newPage) == false) {
              ...
              if (i + 1 <= counter(word + "abbr").get().at(0)) {
                if (
                  i + 1 == counter(word + "abbr").get().at(0)
                    and counter(page).at(locate(label(word + labelText + str(i + 1)))) == newPage
                ) {
                  pagesLink += link(label(word + labelText + str(i)))[#text(fill: rgb("#2C5E77"))[#str(newPage.at(0))].]
                } else {
                  pagesLink += link(
                    label(word + labelText + str(i)),
                  )[#text(fill: rgb("#2C5E77"))[#str(newPage.at(0))], ]
                }
              } else {
                pagesLink += link(label(word + labelText + str(i)))[#text(fill: rgb("#2C5E77"))[ #str(newPage.at(0))].]
              }
