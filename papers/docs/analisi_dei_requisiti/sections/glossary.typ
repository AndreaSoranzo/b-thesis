...
#let generateGlossaryFromYaml(contents) = {
  for (word, definition) in contents {
    ...
    let w=[#figure(kind: "word_definition",supplement: "Definizione",)[*#upper(word.at(0))#word.slice(1) :*]#label(word)]
      ...
      while i <= counter(word).get().at(0) {
        ...
        if(pages.contains(newPage)==false) {
          ...
          if(i+1 <= counter(word).get().at(0)) {
            if(i+1==counter(word).get().at(0) and counter(page).at(locate(label(word+"link"+str(i+1))))==newPage) {
...
#let glos(word) = {
  ...
  context(
    ...
    if(abbrOnly==true) {
      output=text(style: "italic",fill: rgb("#2C5E77"))[#link(label(labelText))[#linkText#sub(size: 0.7em)[G]]#label(word+"abbr"+str(counter(word+"abbr").get().at(0)))]
    } else {
      output=text(style: "italic",fill: rgb("#2C5E77"))[#link(label(labelText))[#linkText#sub(size: 0.7em)[G]]#label(word+"link"+str(counter(word).get().at(0)))]
    },
    output
  ).at(4)
