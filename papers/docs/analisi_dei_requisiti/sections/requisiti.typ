...
#table(
  ...
  [RF#context { Rid(RF) }], [L'utente deve visualizzare il CAPTCHA nel caso il WAF rilevi troppe richieste in brevi periodi di tempo], [M], [UC1.2],
  [RF#context { Rid(RF) }], [L'utente deve visualizzare una pagina di blocco nel caso in cui una richiesta venga rilevata sospetta e venga bloccata dal WAF], [M], [UC1.1],
  // admin - vs
  [RF#context { Rid(RF) }], [L'admin F5 deve gestire e creare dei virtual server in modo da permettere gli utenti di collegarsi all'applicazione web], [M], [UC2],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve definire le regole di protezione da attacchi da parte di BOT e BOTNET], [M], [UC2.3],
  // admin - policy
  [RF#context { Rid(RF) }], [L'admin F5 deve gestire e creare le policy da assegnare al WAF in modo che riesca a filtrare le richieste sospette], [M], [UC2.4],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire le regole per la protezione dei dati personali contenute nelle risposte del server], [M], [UC2.4.1],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire le regole per il rilevamento di numeri di carte di credito], [M], [UC2.4.1.1],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire le regole per il rilevamento di codici fiscali], [M], [UC2.4.1.2],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire regole da applicare ai parametri che vengono passati al server tramite URL], [M], [UC2.4.2],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve definire quali meta-caratteri sono consentiti per parametro], [M], [UC2.4.2.3],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire quali URL sono permessi e visibili all'utente], [M], [UC2.4.3.1],
  [RF#context { Rid(RF) }], [L'admin F5 deve definire quali URL non sono permessi e non visibili all'utente], [M], [UC2.4.3.2],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve definire gli URL accessibili solo dopo l'autenticazione dell'utente], [M], [UC2.4.4.2],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve creare regole per filtrare determinati tipi di richieste HTTP], [M], [UC2.4.5],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve definire regole per garantire l'autenticazione dei cookie], [M], [UC2.4.5.3],
  [RF#context { Rid(RF) }], [L'admin F5 deve gestire come vengono mitigati gli attacchi di tipo brute force], [M], [UC2.4.6],
  ...
  [RF#context { Rid(RF) }], [L'admin F5 deve instaurare il collegamento con il server syslog per memorizzare tutti i log creati dal WAF], [D], [UC3.1],
)
...
#table(
  ...
  [RQ#context { Rid(RQ) }], [Il WAF deve essere in grado di bloccare gli attacchi di tipo JS e SQL injection], [M], [Progetto],
  [RQ#context { Rid(RQ) }], [Il WAF deve essere in grado di bloccare gli attacchi di manomissione dei cookie e delle sessioni utente], [M], [Progetto],
  ...
  [RQ#context { Rid(RQ) }], [Il WAF deve essere in grado di mascherare le informazioni sensibili degli utenti], [M], [Progetto],
  [RQ#context { Rid(RQ) }], [Il WAF deve essere in grado di rilevare e bloccare attacchi DoS], [M], [Progetto],
  [RQ#context { Rid(RQ) }], [Il WAF deve essere in grado di rilevare e bloccare attacchi di brute force], [M], [Progetto],
)
