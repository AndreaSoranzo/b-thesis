...
#table(
  ...
  row-gutter: 4pt,
  rows: 40pt,
  [*Attore primario*], [Utente che ha accesso all'applicazione],
  [*Descrizione*], [L'utente può visualizzare e utilizzare l'applicazione normalmente],
  [*Precondizioni*], [L'utente ha accesso all'applicazione Il server e il WAF sono operativi],
  [*Postcondizioni*], [L'utente è in grado di visualizzare e utilizzare l'applicazione],
  [*Estensioni*], [UC1.1 - Visualizza blocking page \ UC1.2 - Visualizza captcha],
)
...
#table(
  ...
  [*Attore primario*], [Utente che ha accesso all'applicazione],
  [*Descrizione*],
  [Se un utente tenta di effettuare una richiesta HTTP che risulta sospetta o illegale (es. accesso a risorse proibite o inserimento di attacchi web), verrà visualizzata la pagina di blocco impostata dall'admin del firewall.],

  [*Precondizioni*], [L'utente ha effettuato una richiesta HTTP al server],
  [*Postcondizioni*], [La richiesta non viene mandata al server \ L'utente visualizza la pagina di blocco],
  [*Generalizzazioni*], [1.1.1 - Visualizza pagina html di errore \ 1.1.2 - Visualizza messaggio di errore],
)
...
#table(
  ...
  [*Precondizioni*], [L'utente ha effettuato una richiesta HTTP al server],
  [*Postcondizioni*], [L'utente reindirizzato ad una nuova pagina],
)
...
#table(
  ...
  [*Attore primario*], [Utente che ha accesso all'applicazione],
  [*Descrizione*],
  [Se il sistema rileva un potenziale attacco di tipo DoS o un'attività sospetta riconducibile a una botnet, \ viene presentata la pagina di test captcha per bloccare le richieste malevole, poiché bot automatizzati difficilmente riusciranno a completarlo.],

  [*Precondizioni*],
  [Vengono rilevate molte richieste in periodi di tempo brevi dallo stesso sorgente o da user agent sospetti],

  [*Postcondizioni*], [Viene presentata la pagina di test captcha],
)
...
#table(
  ...
  row-gutter: 4pt,
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*],
  ...
  [*Precondizioni*], [Il servizio di BIG-IP LTM deve essere installato],
  [*Postcondizioni*], [Viene assegnato un nuovo virtual server al servizio BIG-IP LTM],
)
...
#table(
  ...
  [*Precondizioni*], [Il servizio di BIG-IP LTM deve essere installato],
  [*Postcondizioni*], [Viene creata una nuova pool di server],
)
...
#table(
  ...
  [*Precondizioni*], [Il servizio di BIG-IP ASM deve essere installato],
  [*Postcondizioni*], [Viene creata una nuova regola di protezione DoS],
)
...
#table(
  ...
  [*Precondizioni*], [Il servizio di BIG-IP ASM deve essere installato],
  [*Postcondizioni*], [Viene creata una nuova policy],
)
...
#table(
  ...
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*],
  [L'admin può definire regole di data guard che permettono al WAF di identificare e censurare automaticamente informazioni sensibili degli utenti all'interno delle \ risposte HTTP da parte del server. Questo impedisce a potenziali attaccanti di visualizzare tali dati],
...
#table(
  ...
  [*Precondizioni*], [La funzione di rilevamento di carta di credito è disattivata],
  [*Postcondizioni*], [La funzione di rilevamento di carta di credito è attiva],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può attivare la funzione per l'identificazione di codici fiscali nelle risposte HTTP],
  [*Precondizioni*], [La funzione di rilevamento di codici fiscali è disattivata],
...
#table(
  ...
  [*Precondizioni*],
  [La policy alla quale si vuole assegnare queste regole deve essere presente \ Tutti i parametri URL sono permessi indipendentemente dalla loro struttura],

  [*Postcondizioni*], [La policy possiede le regole riguardanti la struttura dei parametri URL],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può decidere che tipo di valore deve essere il parametro URL nella richiesta HTTP],
  [*Precondizioni*], [Il tipo di valore è impostato su "qualsiasi"],
  [*Postcondizioni*], [Il tipo di valore è impostato su quello deciso dall'admin],
)
...
#table(
  ...
  [*Precondizioni*], [La lunghezza massima è impostata su "qualsiasi"],
  [*Postcondizioni*], [La lunghezza massima è impostata al valore deciso dall'admin],
)
...
#table(
  ...
  [*Precondizioni*], [Tutti i metacaratteri non sono permessi],
...
#table(
  ...
  [*Precondizioni*],
  [La policy alla quale si vuole assegnare queste regole deve essere presente \ Tutti gli URL sono navigabili],

  [*Postcondizioni*], [La policy possiede le regole riguardanti quali URL sono permessi e quali no],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire quali URL sono permessi],
  [*Precondizioni*], [Tutti gli URL sono permessi],
  [*Postcondizioni*], [Solo gli URL elencati dall'admin sono permessi],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire quali URL non sono permessi],
  [*Precondizioni*], [Tutti gli URL sono permessi],
  [*Postcondizioni*], [Gli URL elencati dall'admin non sono permessi],
)
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [La policy possiede le regole e condizioni per identificare se un utente risulta non autenticato],
)
...
#table(
  ...
  [*Precondizioni*], [Nessuna condizione definita, quindi tutti i tentativi di accesso falliscono],
  [*Postcondizioni*], [Vengono applicate le condizioni di successo definite dall'admin],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin deve definire gli URL che richiedono l'autenticazione dell'utente per essere accessibili.],
  [*Precondizioni*], [Nessun URL è definito],
  [*Postcondizioni*], [Vengono applicati dei controlli di sessione agli URL definiti],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin deve definire quali URL corrispondono alle pagine di login e logout],
  [*Precondizioni*], [Nessuna pagina è definita],
  [*Postcondizioni*], [Vengono definiti gli URL di login e logout],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire i parametri necessari per l'autenticazione dell'utente su un determinato URL],
  [*Precondizioni*], [Non esistono parametri di login definiti per l'URL],
  [*Postcondizioni*], [Vengono definiti i parametri di login per l'URL],
)
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [La policy possiede le regole per individuare richieste HTTP sospette],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire gli header HTTP permessi in una richiesta],
  [*Precondizioni*], [La lista degli header permessi contiene solo "PUT", "GET", "POST"],
  [*Postcondizioni*], [Viene applicata la lista definita dall'admin],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire i file permessi nelle richieste e risposte HTTP],
  [*Precondizioni*], [La lista è vuota],
  [*Postcondizioni*], [Viene applicata la lista definita dall'admin],
)
...
#table(
  ...
  [*Precondizioni*], [Non sono presenti regole],
  [*Postcondizioni*], [Vengono applicate le regole definite dall'admin],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può gestire e creare regole per individuare e mitigare attacchi di tipo brute force],
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può gestire e creare regole per individuare e mitigare attacchi di tipo CSRF],
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [La policy possiede regole per individuare e mitigare attacchi di tipo CSRF],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può gestire e creare regole per individuare e mitigare attacchi di tipo SSRF],
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [La policy possiede regole per individuare e mitigare attacchi di tipo SSRF],
)
...
#table(
  ...
  inset: 10pt,
  [*Attore primario*], [Admin del WAF F5],
  [*Descrizione*], [L'admin può definire ulteriori attack signatures oltre a quelle già proposte dal modulo BIG-IP ASM],
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*],
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [Il modulo di IP Intelligence viene attivato sulla policy selezionata],
)
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [Viene applicata la configurazione scelta dall'admin],
)
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*],
  [Vengono create delle attack signature inerenti alle tecnologie elencate \ Viene aggiornato l'elenco delle tecnologie della policy],
)
...
#table(
  ...
  [*Precondizioni*], [La policy alla quale si vuole assegnare queste regole deve essere presente],
  [*Postcondizioni*], [Viene aggiornato lo stile delle pagine di blocco],
)
...
#table(
  ...
  [*Precondizioni*], [L'amministratore desidera creare profili personalizzati per ottimizzarne l'organizzazione],
  [*Postcondizioni*], [Il profilo di logging viene applicato al firewall],
)
...
#table(
  ...
  [*Postcondizioni*], [Il server esterno riceve i log impostati dall'admin tramite il profilo di logging],
)
