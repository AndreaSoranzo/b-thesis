...
#figure(image("../img/study/learning-blocking.png", width: 90%), caption: [Learning and Blocking Settings]) <lms>
Questo pannello in [@lms] mostra tutti i tipi di richieste categorizzate come illegali dal WAF. Oltre a modificare l'opzione di "learn", come spiegato in precedenza, puoi anche
...
cartelle specifiche.
Al momento dell'installazione, il modulo ASM importa una vasta gamma di firme predefinite come quelle elencate in [@ats], che coprono le tecnologie più diffuse e le relative vulnerabilità, assicurando una
...
Per mitigare questi rischi, è possibile specificare quali metodi HTTP sono consentiti per una determinata richiesta come mostrato in [@head], limitando così le operazioni che un utente può
eseguire.
...
tutte le pagine che necessitano di autenticazione per essere accessibili nella sezione "Authenticated URLs" della [@ses].
...
aziende un aggiornamento costante sulle principali minacce.
Infatti come mostrato in [@owaspmap] la classifica viene aggiornata ogni anno.
La sua finalità è quella di sensibilizzare gli sviluppatori, incoraggiandoli a integrare pratiche di sicurezza
...
#figure(image("../img/study/owasp.png", width: 70%), caption: [OWASP top 10 2021])<top>
Tale scelta è stata motivata dalla solidità della
documentazione disponibile e dalla sua rilevanza, in quanto le vulnerabilità principali non hanno subito variazioni sostanziali rispetto alle versioni più recenti. La
